...
int sum_of_doubly_divisibles(int n, int l, int m)
{
    ...
    while (j <= n)
    {
        if (is_multiple_of(i, m) && is_multiple_of(i, l))
        {
            total += i;
            j++;
        }
...
Canvas color_corners(Canvas c)
{
    ...
    c = canvas_set_black(c, canvas_width(c) - 1, 0);
    c = canvas_set_black(c, 0, canvas_height(c) - 1);
    c = canvas_set_black(c, canvas_width(c) - 1, canvas_height(c) - 1);
...
{
    int count = 1;
    for (int i = canvas_height(c) - 1; i >= 0; i--)
    {
        for (int j = 0; j < count; j++)
        {
            c = canvas_set_black(c, j, i);
        }
        if (count < canvas_width(c))
        {
            count++;
        }
    }
...
Canvas draw_rectangle(Canvas c, int x, int y, int width, int height)
{
    ...
    {
        if (i >= 0 && i < canvas_width(c))
        {
            for (int j = y; j > y - height; j--)
            {
                if (j >= 0 && j < canvas_height(c)){
                    c = canvas_set_black(c, i, j);
