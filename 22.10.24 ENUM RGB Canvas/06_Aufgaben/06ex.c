...
Canvas swatch_red_green(Canvas c) {
    ...
    for (int i = 0; i < canvas_width(c); i++) {
        for (int j = 0; j < canvas_height(c); j++) {
            c = canvas_set_r(c, i, j, (float)i / (canvas_width(c) - 1));
            c = canvas_set_g(c, i, j, (float)j / (canvas_height(c) - 1));
        }
    }
...
Canvas swatch_rgb(Canvas c, ColorChannel channel_x, ColorChannel channel_y) {
    ...
    for (int i = 0; i < canvas_width(c); i++) {
        for (int j = 0; j < canvas_height(c); j++) {
            float x = normalize(i, canvas_width(c));
            float y = normalize(j, canvas_height(c));
            c = canvas_set_channel(c, i, j, channel_x, x);
            c = canvas_set_channel(c, i, j, channel_y, y);
        }
    }
    return c;
}
...
Canvas canvas_to_gray(Canvas c) {
    for (int i = 0; i < canvas_width(c); i++) {
        for (int j = 0; j < canvas_height(c); j++) {
            RGB color;
            color.r = canvas_get_r(c, i, j);
            color.g = canvas_get_g(c, i, j);
            color.b = canvas_get_b(c, i, j);
            color = rgb_to_gray(color);
            c = canvas_set_rgb_struct(c, i, j, color);
        }
    }
    return c;
}
