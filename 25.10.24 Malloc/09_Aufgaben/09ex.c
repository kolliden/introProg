...
Um den Code mit `fsanitize` auszuführen (gleiche Funktionalität wie valgrind), führen Sie die folgenden Befehle aus:
 Für Zugriffsfehler: 
 clang -std=c11 -g -Wall 09ex_test.c -o 09ex_test.o -lm -fsanitize=address && ./09ex_test.o
 Für Speicherfehler: 
 clang -std=c11 -g -Wall 09ex_test.c -o 09ex_test.o -lm -fsanitize=memory && ./09ex_test.o
...
uint32_t *return_pointer(uint32_t x) {
    //return &x; // Das ist nicht gültig, da x lokal ist und nach dem Verlassen der Funktion nicht mehr existiert.
...
PileOfPancakes *fruit_crepe_ptr() {
    ...
    if (p != NULL) {
        ...
        if (p->further_layers != NULL) {
            p->further_layers->layer = Fruit;
            p->further_layers->further_layers = NULL;
        }
    }
    return p;
}
...
PileOfPancakes mille_crepes(uint32_t n) {
    ...
    PileOfPancakes *current = &p; //Set the current layer to the first layer ADDY
    for (uint32_t i = 1; i < n; i++){
        current->further_layers = malloc(sizeof(PileOfPancakes)); //Allocate memory for the next layer
        if (current->further_layers != NULL) {
            current->further_layers->layer = ActualPancake;
            current->further_layers->further_layers = NULL;
...
uint16_t *create_dynamic_array(size_t x) {
    ...
    if (array != NULL) { // Check if malloc was successful
